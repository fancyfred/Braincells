...
export default async function Page({
  ...
  const tags = Array.from(new Set(olympicsFacts.flatMap((fact) => fact.tags))).sort();
  ...
  if (!selectedTag && tags.length > 0 && !UNSPLASH_DISABLED) {
    redirect(`/olympics?tag=${encodeURIComponent(tags[0])}`);
  }

  return (
    <SiteLayout>
      <section className="shell">
        <h1>Olympics Facts</h1>
        <LayoutSelector />
        <div className="facts-layout">
          <aside className="facts-sidebar filter-left">
            <FactFilter facts={olympicsFacts} selectedTag={selectedTag} />
          </aside>
          ...
          <main className="facts-content">
            <FactList facts={olympicsFacts} selectedTag={selectedTag} />
          </main>
