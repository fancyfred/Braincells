...
export default async function Page({
  ...
  const tags = Array.from(new Set(seinfeldFacts.flatMap((fact) => fact.tags))).sort();
  ...
  if (!selectedTag && tags.length > 0 && !UNSPLASH_DISABLED) {
    redirect(`/seinfeld?tag=${encodeURIComponent(tags[0])}`);
  }

  return (
    <SiteLayout>
      <section className="shell">
        <h1>Seinfeld Facts</h1>
        <LayoutSelector />
        <div className="facts-layout">
          <aside className="facts-sidebar filter-left">
            <FactFilter facts={seinfeldFacts} selectedTag={selectedTag} />
          </aside>
          ...
          <main className="facts-content">
            <FactList facts={seinfeldFacts} selectedTag={selectedTag} />
          </main>
