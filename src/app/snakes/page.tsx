...
export default async function Page({
  ...
  const tags = Array.from(new Set(snakesFacts.flatMap((fact) => fact.tags))).sort();
  ...
  if (!selectedTag && tags.length > 0 && !UNSPLASH_DISABLED) {
    redirect(`/snakes?tag=${encodeURIComponent(tags[0])}`);
  }
      ...
      <section className="shell">
        ...
        <div className="facts-layout">
          <aside className="facts-sidebar filter-left">
            <FactFilter facts={snakesFacts} selectedTag={selectedTag} />
          </aside>
          ...
          <main className="facts-content">
            <FactList facts={snakesFacts} selectedTag={selectedTag} />
          </main>
